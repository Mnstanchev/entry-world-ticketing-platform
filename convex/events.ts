...
// Internal version of checkAvailability for use within mutations
export const internalCheckAvailability = internalQuery({
  args: { eventId: v.id("events") },
  handler: async (ctx, { eventId }) => {
    const event = await ctx.db.get(eventId);
    if (!event) throw new Error("Event not found");
...
export const joinWaitingList = mutation({
  ...
  handler: async (ctx, { eventId, userId }): Promise<{
    ...
    const { available }: { available: boolean } = await ctx.runQuery(internal.events.internalCheckAvailability, { eventId });
